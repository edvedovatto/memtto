...
export const EntryCard = memo(function EntryCard({
  ...
  const inner = (
    <div className={cn(
      "group overflow-hidden rounded-xl border bg-surface transition-colors duration-200 hover:bg-surface-hover",
      selectionMode && isSelected ? "border-accent/50 bg-accent/5" : "border-border"
    )}>
  ...
  if (selectionMode) {
    return (
      <div className="cursor-pointer" onClick={() => onToggleSelect?.(entry.id)}>
        {inner}
      </div>
    );
  }

  return <Link href={`/entry/${entry.slug}`}>{inner}</Link>;
});
